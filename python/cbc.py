...
def cipher_block_chaining(plaintext, key, init_vec, block_size, block_enc):
    ...
    for i in range(len(plaintext) / block_size + 1):
        ...
        if start >= len(plaintext):
            break
        end = min(block_size + start, len(plaintext))
        block = plaintext[start:end]
        pre_cipher = xor_block(block, prev_block[0: len(block)])
        cipher_block = block_enc(pre_cipher, key)
...
def test():
    ...
    assert bits_to_string(
        cipher) == '\xeaJ\x13t\x00\x1f\xcb\xf8\xd2\x032b\xd0\xb6T\xb2\xb1\x81\xd5h\x97\xa0\xaeogtNi\xfa\x08\xca\x1e'

    cipher = cipher_block_chaining(plaintext, key, iv, 128, non_encoder)
    assert bits_to_string(
        cipher) == 'wW/i\x05\rJQ]\x05\\\r\x05\x0e_G\x03 @Ilkj3$%/hd\x00\x00\x00'

    cipher = cipher_block_chaining(plaintext, key, iv, 128, xor_encoder)
    assert bits_to_string(
        cipher) == 'C?\x17\x0f+=sb0O37/7|c\x03 @Ilkj3$%/hd9#$'
